...
echo("<div class=col-md-12>");
    echo("<div class=Div1><h4><b># Farmer Bill Details.</b></h4> <br>");
...
$q1="select * from tbl_farmer_bill where fcode='$code' and Date between '$from' and '$to'";
       ...
       if($res)
        {
          ...
          echo("<tr><th>Sr no.</th><th>Date</th><th>Owner name</th><th>Ton</th></tr>");
            while ($row=$res->fetch_assoc()) 
	              {
                    $a=$a+$row['total_amt'];
                    $b=$b+$row['ton'];
                    $r=$row['rate'];
                     echo("<tr><td>$row[bno]</td><td>$row[Date]</td><td>$row[oname]<td>$row[ton]</td></tr>");
                }
