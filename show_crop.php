...
.div h4{
  ...
  <nav class="navbar navbar-expand-lg navbar-light text-white" >
     ...
     // $to=$_POST['to'];    
      ...
      $result = mysqli_query($con, "select fname from tbl_farmer where fcode =$code");
    ...
    echo("<div class=Div1><h4><b># Farmer Crop Details.</b></h4> <br>");
...
$q1="select cno,Date,duration,variety from tbl_crop where fcode=$code";

$res=$con->query($q1) or die("Query Error");

    
   echo("<br>");
    echo("<h2>Crop details</h2>");
       if($res)
        {
          echo("<table id=customers>");
          echo("<tr><th>Sr no.</th><th>Date</th><th>Duration</th><th>Variety</th></tr>");
            while ($row=$res->fetch_assoc()) 
	              {
                     echo("<tr><td>$row[cno]</td><td>$row[Date]</td><td>$row[duration]</td><td>$row[variety]</td></tr>");
                }
